  ...
  async sendMessage(
    ...
    try {
      ...
      const quickReplyGuide =
        '[빠른 답변 보기 지침] 보기는 꼭 필요할 때만, 가끔만 제시하라(매 응답마다 붙이지 말 것). 학생이 막연해할 만한 열린 질문(예: 흥미·강점을 처음 물을 때)에만, 응답의 맨 마지막에 별도의 한 줄로 "[보기] 짧은답변1 | 짧은답변2" 형식으로 최대 2개까지만 제시하라. ' +
        '구체적인 질문이거나 학생이 이미 충분히 답하고 있으면 보기를 붙이지 말라. 정리·추천·표 응답에는 절대 [보기] 줄을 붙이지 말 것. 보기 줄 외에 다른 설명을 덧붙이지 말 것.';
