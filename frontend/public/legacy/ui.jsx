// ui.jsx — Toss-inspired shared component library
// All components assume `toss-theme.css` is loaded (CSS variables).
// Uses inline styles + `Object.assign(window, ...)` to expose names globally.

// ────────────────────────────────────────────────────────────────
// useFocusTrap — accessibility: trap Tab focus inside an open overlay,
// focus the first focusable on open, restore focus on close, and call
// onEscape when Esc is pressed. Used by BottomSheet / ConfirmDialog / dialogs.
// ────────────────────────────────────────────────────────────────
...
function Button({
  ...
  full, leading, trailing, style, type = 'button', loading = false,
}) {
  const sizes = {
  ...
  const variants = {
    primary: {
      bg: 'var(--brand-500)', color: '#fff',
      hover: 'var(--brand-600)', border: 'transparent',
    },
    secondary: {
      bg: 'var(--neutral-bg)', color: 'var(--fg-strong)',
      hover: '#E8ECEF', border: 'transparent',
    },
    ghost: {
      bg: 'transparent', color: 'var(--fg-default)',
      hover: 'var(--bg-muted)', border: 'transparent',
    },
    outline: {
      bg: '#fff', color: 'var(--fg-strong)',
      hover: 'var(--bg-muted)', border: 'var(--line-strong)',
    },
    danger: {
      bg: 'var(--danger)', color: '#fff',
      hover: '#B91C1C', border: 'transparent',
    },
    brandSoft: {
      bg: 'var(--brand-50)', color: 'var(--brand-600)',
      hover: 'var(--brand-100)', border: 'transparent',
    },
  }[variant] || {};
...
function Chip({ tone = 'neutral', size = 'md', children, leading, style }) {
  ...
  return (
    <span style={{
      ...
      fontSize: sizes.fs, fontWeight: sizes.fw,
    ...
    <Card padding={padding} style={style}>
      {(title || action) && (
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: subtitle ? 4 : 16 }}>
          <div>
            {title && <div style={{ fontSize: 17, fontWeight: 700, color: 'var(--fg-strong)' }} className="kr-heading">{title}</div>}
            {subtitle && <div style={{ fontSize: 13, color: 'var(--fg-muted)', marginTop: 4 }}>{subtitle}</div>}
          </div>
          {action}
        </div>
      )}
      {subtitle && <div style={{ marginTop: 16 }}>{children}</div>}
      {!subtitle && children}
    </Card>
  );
}
...
function EmptyState({ icon, title, body, action }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--fg-strong)' }}>{title}</div>
      {body && <div style={{ fontSize: 13, color: 'var(--fg-muted)', maxWidth: 280, lineHeight: 1.5 }} className="kr-heading">{body}</div>}
...
function ErrorState({ title = '문제가 발생했어요', body, onRetry }) {
  return (
    <EmptyState
      icon={<IcAlert size={24}/>}
      title={title}
      body={body || '잠시 후 다시 시도해주세요.'}
      action={onRetry && <Button variant="secondary" size="sm" leading={<IcRefresh size={14}/>} onClick={onRetry}>다시 시도</Button>}
    />
  );
}
...
function TextInput({ value, onChange, placeholder, type = 'text', leading, trailing, error, style, autoFocus, onKeyDown, readOnly }) {
  ...
  return (
    <div style={{
      ...
      {leading && <span style={{ color: 'var(--fg-muted)' }}>{leading}</span>}
      <input
        type={type}
        value={value}
        autoFocus={autoFocus}
        ...
        style={{
          ...
          fontSize: 15, color: 'var(--fg-strong)',
          minWidth: 0,
        }}
      />
      {trailing}
    ...
    <div style={{
      ...
      <textarea
        ...
        style={{
          ...
          fontSize: 15, color: 'var(--fg-strong)', fontFamily: 'inherit',
...
function FormField({ label, hint, error, required, children, style }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, ...style }}>
      {label && (
        <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg-default)' }}>
          {label}
      ...
      {hint && !error && <div style={{ fontSize: 12, color: 'var(--fg-subtle)' }}>{hint}</div>}
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 44 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{trailing}{help && typeof HelpButton !== 'undefined' && <HelpButton pageId={help}/>}</div>
      </div>
      {title && (
        <div style={{
          fontSize: large ? 28 : 20, fontWeight: 700, color: 'var(--fg-strong)',
          marginTop: large ? 8 : 0, letterSpacing: '-0.4px',
        }} className="kr-heading">{title}</div>
      )}
      {subtitle && (
        <div style={{ fontSize: 14, color: 'var(--fg-muted)' }} className="kr-heading">{subtitle}</div>
      )}
    </div>
  );
}
...
function BackButton({ onClick }) {
  return (
    <button onClick={onClick} aria-label="뒤로" style={{
      width: 40, height: 40, border: 'none', background: 'transparent', cursor: 'pointer',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      borderRadius: 10, color: 'var(--fg-strong)', marginLeft: -8,
    }}>
...
function IconButton({ icon, onClick, badge, ariaLabel, size = 40, ...rest }) {
  return (
    <button onClick={onClick} aria-label={ariaLabel} {...rest} style={{
      width: size, height: size, border: 'none', background: 'transparent', cursor: 'pointer',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      borderRadius: 10, color: 'var(--fg-strong)', position: 'relative',
    }}>
      {icon}
      {badge != null && badge > 0 && (
        <span style={{
      ...
      {items.map(it => {
        ...
        return (
          <button key={it.id} onClick={() => onChange(it.id)} style={{
            ...
            {React.cloneElement(it.icon, { size: 22, strokeWidth: active ? 2.25 : 1.75 })}
            <span style={{ fontSize: 10, fontWeight: active ? 700 : 500, letterSpacing: '-0.2px' }}>{it.label}</span>
          </button>
        );
      })}
    </div>
  ...
  return (
    <Card padding={18} style={style}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span className="num" style={{ fontSize: 24, fontWeight: 700, color: 'var(--fg-strong)', letterSpacing: '-0.5px' }}>{value}</span>
        {delta && (
          <Chip tone={deltaTone} size="sm" style={{ height: 20 }}>
            {delta}
          </Chip>
        )}
      </div>
      {hint && <div style={{ fontSize: 12, color: 'var(--fg-subtle)', marginTop: 6 }}>{hint}</div>}
    </Card>
  );
}
...
function BottomSheet({ open, onClose, title, children, maxHeight = '85%' }) {
  ...
  return (
    <div style={{
      ...
      <div ref={trapRef} role="dialog" aria-modal="true" aria-label={title || '시트'} style={{
        position: 'relative',
        ...
        <div style={{
          ...
          borderRadius: 999, margin: '10px auto 8px',
        }}/>
        {title && (
          <div style={{
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--fg-strong)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{title}</div>
...
function ProgressBar({ value, max = 100, height = 6, color = 'var(--brand-500)' }) {
  ...
  return (
    <div style={{
      height, background: 'var(--neutral-bg)', borderRadius: 999, overflow: 'hidden',
    }}>
      <div style={{
...
function Tabs({ items, activeId, onChange, variant = 'pills' }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--neutral-bg)', borderRadius: 12,
    }}>
      {items.map(it => {
        const active = it.id === activeId;
        return (
...
function ConfirmDialog({ open, title, body, confirmLabel = '확인', cancelLabel = '취소', onConfirm, onCancel, danger, reasonRequired }) {
  ...
  return (
    <div style={{
      ...
      <div ref={trapRef} role="alertdialog" aria-modal="true" aria-label={title} style={{
        position: 'relative', width: '100%', maxWidth: 360,
        background: 'var(--bg-elevated)', borderRadius: 20, padding: 24,
        boxShadow: 'var(--shadow-pop)',
        animation: 'sheetIn 280ms var(--ease-toss)',
      }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--fg-strong)', marginBottom: 8 }} className="kr-heading">{title}</div>
        {body && <div style={{ fontSize: 14, color: 'var(--fg-muted)', lineHeight: 1.5, marginBottom: reasonRequired ? 12 : 20 }} className="kr-heading">{body}</div>}
        {reasonRequired && (
          <FormField label="사유 (감사 로그에 기록됩니다)" required style={{ marginBottom: 20 }}>
            <Textarea value={reason} onChange={setReason} placeholder="조치 사유를 4자 이상 입력해주세요" rows={3}/>
          </FormField>
        )}
        <div style={{ display: 'flex', gap: 8 }}>
          <Button variant="secondary" size="md" full onClick={onCancel}>{cancelLabel}</Button>
          <Button variant={danger ? 'danger' : 'primary'} size="md" full
            disabled={!canConfirm}
...
Object.assign(window, {
  useFocusTrap,
  Button, Chip, Card, SectionCard, EmptyState, Skeleton, ErrorState,
  TextInput, Textarea, FormField,
  ScreenHeader, BackButton, IconButton, MobileBottomNav,
  MetricCard, Avatar, BottomSheet, ListRow, ProgressBar, Tabs,
  ConfirmDialog, Toast,
});
